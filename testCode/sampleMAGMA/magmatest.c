...
int main() {
    ...
    //printf("%f\n", dotProd);
    int nrhs = 1; // Cols of B, always 1 in our case
    int numEqs = 3; // Size of side of matrix, or number of linear equations
    int lda = numEqs;
    int ldb = lda;
    ...
    // investigate
    magma_int_t info = magma_sgesv(numEqs, nrhs, matA, 
                                    lda, ipiv, vecB, lda, &info1);
